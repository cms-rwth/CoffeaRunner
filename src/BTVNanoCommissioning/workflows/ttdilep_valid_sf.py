...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        events.Muon = events.Muon[
            (events.Muon.pt > 30) & (abs(events.Muon.eta < 2.4)) & (events.Muon.tightId) & (events.Muon.pfRelIso04_all < 0.12)
        ]
            ...
            if "Deep" in histname and "btag" not in histname:
                ...
                for i in range(2):
                    ...
                    if str(i) in histname:
                        h.fill(flav=flatten(genflavor[:, i]), syst="noSF", discr=flatten(np.where(sel_jet[histname.replace(f"_{i}","")] < 0, -0.2, sel_jet[histname.replace(f"_{i}","")])),weight=weights.weight()[event_level])
                        if not isRealData and self.isCorr:
                            for syst in disc_list[histname.replace(f"_{i}", "")][i].keys():
                                h.fill(
                                    flav=flatten(genflavor[:, i]),
                                    syst=syst,
                                    discr=flatten(np.where(sel_jet[histname.replace(f"_{i}","")] < 0, -0.2, sel_jet[histname.replace(f"_{i}","")])),
                ...
                h.fill(flatten(sel[histname.replace("ele_","")]),weight=weights.weight()[event_level])
