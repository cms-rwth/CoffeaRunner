            ...
            if "Reco" in sf:
                ...
                sfs_low = np.where(
                    ...
                    correct_map["EGM"][list(correct_map["EGM"].keys())[0]].evaluate(
                        sf[sf.find(" ") + 1 :], "sf", "RecoBelow20", ele_eta, ele_pt_low
                    ),
                    1.0,
                )
                sfs = np.where(
                ...
                if syst:
                    ...
                    weights.add(sf.split(" ")[0], sfs)

            else:
                sfs = correct_map["EGM"][list(correct_map["EGM"].keys())[0]].evaluate(
                    sf[sf.find(" ") + 1 :],
                    "sf",
...
def muSFs(mu, correct_map, weights, syst=True, isHLT=False):
    ...
    for sf in correct_map["MUO_cfg"].keys():
        ...
        if (
            ...
            sfs = correct_map["MUO"][correct_map["MUO_cfg"][sf]].evaluate(
                sf[sf.find(" ") + 1 :], mu_eta, mu_pt, "sf"
            )
            if syst:
                sfs_up = sfs + correct_map["MUO"][correct_map["MUO_cfg"][sf]].evaluate(
                    sf[sf.find(" ") + 1 :], mu_eta, mu_pt, "systup"
                )
                sf_down = sfs - correct_map["MUO"][correct_map["MUO_cfg"][sf]].evaluate(
            ...
            if "mu" in sf:
                ...
                if syst:
                    sfs_up = sfs + correct_map["MUO_custom"][f"{sf_type}_error"](
                        mu_eta, mu_pt
                    )
                    sf_down = sfs - correct_map["MUO_custom"][f"{sf_type}_error"](
                        mu_eta, mu_pt
                    )
                    weights.add(sf.split(" ")[0], sfs, sfs_up, sfs_down)
