...
class Configurator():
    def __init__(self, cfg, overwrite_output_dir=None, plot=False, plot_version=None):
        ...
        if "categories" in self.cfg.keys():self.load_cuts_and_categories()


        ## Weights configuration
        self.weights_split_bycat = False 
        self.weights_config = {}
        if "categories" in self.cfg.keys():self.weights_config_bycat =  { c: {}for c in self.categories.keys()}
        else :self.weights_config_bycat=None
        
        if "weights" in self.cfg.keys():self.load_weights_config()
                    ...
                    for cat, weights in s_wcfg["bycategory"].items():
                        for weight_name,weight_list in weights.items():
                            if isinstance(weight_name, str):
                                ...
                                self.weights_config_bycat[cat][weight_name][sample] = weight_list
                            else:
                                raise NotImplementedError()
