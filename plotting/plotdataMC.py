...
if len(arg.input.split(","))>1:
    output = {
            i: load(arg.input.split(",")[i])
            for i in arg.input.split(",")
        }
else:
...
for f in output.keys():
    ...
    if 'sumw' not in output[f].keys():
        ...
        mergemap['mc']=[m for m in output[f].keys() if 'Run' not in m and 'data' not in m]
    else :
...
if "Wc" in arg.phase:
    ...
    nj = 2
if "njet" in arg.discr_list.split(",") or "nbjet" in arg.discr_list.split(",") or "mu" in arg.discr_list.split(","):
    nj = 1


for discr in arg.discr_list.split(","):
    if 'flav' in collated["mc"][discr].axes.name and 'syst' in collated["mc"][discr].axes.name:
        scale_sf = np.sum(collated["mc"][discr][{"syst":"SF","flav":sum}].values()) / np.sum(collated["mc"][discr][{"syst":"noSF","flav":sum}].values())
    else:
        scale_sf = 1.0
    if 'flav' in collated["mc"][discr].axes.name and 'syst' in collated["mc"][discr].axes.name:
        print("============> fraction of each flavor in MC")
        print("b:%.3f",np.sum(collated["mc"][discr][{"syst":"SF","flav":3}].values())/ np.sum(collated["mc"][discr][{"syst":"SF","flav":sum}].values()))
        print("c:%.3f", np.sum(collated["mc"][discr][{"syst":"SF","flav":2}].values())/ np.sum(collated["mc"][discr][{"syst":"SF","flav":sum}].values()))
        print("pu:%.3f",np.sum(collated["mc"][discr][{"syst":"SF","flav":1}].values())/ np.sum(collated["mc"][discr][{"syst":"SF","flav":sum}].values()))
        print("l:%.3f",np.sum(collated["mc"][discr][{"syst":"SF","flav":0}].values())/ np.sum(collated["mc"][discr][{"syst":"SF","flav":sum}].values()))
        print("============")
    ...
    hep.cms.label("Preliminary",
                    data=True,
                    lumi=arg.lumi / 1000.0,
                    loc=0,
                    ax=ax )
    
    if 'flav' in collated["mc"][discr].axes.name and 'syst' in collated["mc"][discr].axes.name:
        
        err_up = collated["mc"][discr][{"syst":"SFup","flav":sum}].values()-collated["mc"][discr][{"syst":"SF","flav":sum}].values()
        err_dn = collated["mc"][discr][{"syst":"SF","flav":sum}].values()-collated["mc"][discr][{"syst":"SFdn","flav":sum}].values()
        if "C" in discr  :  ## scale uncertainties for charm tagger by 2
            err_up = np.sqrt(np.add(np.power(err_up,2),np.power(2*(collated["mc"][discr][{"syst":"SFup","flav":4}].values()-collated["mc"][discr][{"syst":"SF","flav":4}].values()),2)))
            err_dn = np.sqrt(np.add(np.power(err_dn,2),np.power(2*(collated["mc"][discr][{"syst":"SF","flav":4}].values()-collated["mc"][discr][{"syst":"SFdn","flav":4}].values()),2)))
        ...
        rax.errorbar(
                    ...
                    y= hdata.values() / collated["mc"][discr][{"syst":"SF","flav":sum}].values(),
                    yerr=ratio_uncertainty(hdata.values(), collated["mc"][discr][{"syst":"SF","flav":sum}].values()),
                    color="k",
                    linestyle="none",
                    marker="o",
                    elinewidth=1,
        ...
        rax.errorbar(
                    x= hdata.axes[0].centers,
                    y= hdata.values() / collated["mc"][discr][{"syst":"noSF","flav":sum}].values(),
                    yerr=ratio_uncertainty(hdata.values(), collated["mc"][discr][{"syst":"noSF","flav":sum}].values()),
                    color="tab:brown",
                    linestyle="none",
                    marker="o",
                    elinewidth=1,
                    )  
        stat_denom_unc = ratio_uncertainty(hdata.values(), collated["mc"][discr][{"syst":"noSF","flav":sum}].values())
        ax.fill_between(
            hdata.axes.edges,
            np.ones(stat_denom_unc[0])-numpy.r_[stat_denom_unc[0], stat_denom_unc[0, -1]],
            np.ones(stat_denom_unc[0])+numpy.r_[stat_denom_unc[1], stat_denom_unc[1, -1]],
        ...
        rax.errorbar(
                    ...
                    y= collated["data"][discr][{"flav":sum}].values() / collated["mc"][discr][{"flav":sum}].values(),
                    yerr=ratio_uncertainty(collated["data"][discr][{"flav":sum}].values(), collated["mc"][discr][{"flav":sum}].values()),
                    color="k",
                    linestyle="none",
                    marker="o",
                    elinewidth=1,
