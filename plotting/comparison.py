...
for f in output.keys():
    if 'sumw' not in output[f].keys():
        ...
        for c in args.compared.split(","):mergemap[c]=[m for m in output.keys() if c==m]
collated = collate(output,mergemap)
### style settings
if 'Run' in args.ref:hist_type="errorbar"
else:hist_type="step"
...
for discr in args.discr_list:
    if args.sepflav:#split into 3 panels for different flavor
        ...
        if 'syst' in collated[args.ref][discr].axes.name:allaxis['syst']:
            laxis['syst']='noSF'
            laxis['syst']='noSF'
            laxis['syst']='noSF'
            laxis['syst']='noSF'
        ...
        for c in args.compared.split(","):
            ...
            rax.errorbar(
                ...
                y= (collated[c][discr][laxis].values()+collated[c][discr][puaxis].values()) / (collated[args.ref][discr][laxis].values()+collated[args.ref][discr][puaxis].values()),
                yerr=ratio_uncertainty((collated[c][discr][laxis].values()+collated[c][discr][puaxis].values()) , collated[args.ref][discr][laxis].values()+collated[args.ref][discr][puaxis].values()),
                color="b",
                linestyle="none",
                marker=markers[c+1],
                ) 
            rax2.errorbar(
                ...
                y= collated[c][discr][caxis].values() / collated[args.ref][discr][caxis].values(),
                yerr=ratio_uncertainty(collated[c][discr][caxis].values() , collated[args.ref][discr][caxis].values()),
                color="g",
                linestyle="none",
                marker=markers[c+1],
                ) 
            rax3.errorbar(
                ...
                y= collated[c][discr][baxis].values() / collated[args.ref][discr][baxis].values(),
                yerr=ratio_uncertainty(collated[c][discr][baxis].values() , collated[args.ref][discr][baxis].values()),
                color="r",
                linestyle="none",
                marker=markers[c+1],
                ) 
        ...
        ax.add_artist(at)
        hep.mpl_magic(ax=ax)
        fig.savefig(f"plot/BTV/{args.phase}_{args.ext}_{time}/compare_{args.phase}_inclusive{discrs}.png" )
        fig.savefig(f"plot/BTV/{args.phase}_{args.ext}_{time}/compare_{args.phase}_inclusive{discrs}.pdf" )
        ...
        ax.set_xticklabels(ax.get_xticklabels(), fontsize=0)
        allaxis={}
        if 'flav' in collated[args.ref][discr].axes.name:allaxis['flav']=sum
        if  'syst' in collated[args.ref][discr].axes.name:allaxis['syst']=sum
        hep.histplot(
        ...
        for c in args.compared.split(",") :
            rax.errorbar(
                    ...
                    y= collated[c][discr][allaxis].values() / collated[args.ref][discr][allaxis].values(),
                    yerr=ratio_uncertainty(collated[c][discr][allaxis].values() , collated[args.ref][discr][allaxis].values()),
                    color="k",
                    linestyle="none",
                    marker="o",
                    elinewidth=1,
